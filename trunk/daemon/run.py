...
if True:#q.last_cheked<time.time()-5 or q.last_cheked==-1:
	...
	if not (q.entry_set.filter(native_id=e.id)):
	    en=Entry(native_id=e.id,title=e.title,summary=e.description,url=e.link,feed=q,downloaded=datetime.now(),created=datetime.utcfromtimestamp(time.mktime(e.updated_parsed)))
	    en.save()
	    count+=1
